...
def plot_learning_curve(logfile_dir, result_lists):
	...
	print(logfile_dir)
	for _, (result_type, result_data) in enumerate(result_lists.items()):
		plt.plot([_ for _ in range(1, len(result_data) + 1)], result_data,'r')
		if 'loss' in result_type:
		...
		plt.savefig(os.path.join(logfile_dir, result_type + '.png'))
		plt.close()
